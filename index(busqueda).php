...
$url1 = 'http://148.204.9.88:8090/newFindRecords?pass=1208&personId=-1&startTime=' . $startDate . '%2000:01:00&endTime=' . $endDate . '%2023:59:59&length=1000&model=-1&order=1&index=0';
...
function processRecords($records, $nameFilter = '') {
    ...
    foreach ($records as $record) {
        ...
        $formattedTime = $dateTime->format('H:i'); // Formato solo de hora
...
function renderTable($groupedRecords, $location) {
    ...
    $html .= '<table><tr><th>Fecha</th><th>Nombre</th><th>Hora (Evento Inicial)</th><th>Hora (Eventos Intermedios)</th><th>Hora (Evento Final)</th><th>Horas Totales</th></tr>';
    
    foreach ($groupedRecords as $date => $persons) {
        foreach ($persons as $personId => $personData) {
            $records = $personData['records'];
            $entry = count($records) > 0 ? $records[0]['time'] : '-';
            $exit = count($records) > 1 ? $records[count($records) - 1]['time'] : '-';
            $intermediateHours = count($records) > 2 ? implode(', ', array_map(function($r) {
                return $r['time'];
            }, array_slice($records, 1, -1))) : '-'; // Obtener horas intermedias
